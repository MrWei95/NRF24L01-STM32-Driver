...
void OLED_WR_Cmd(uint8_t cmd)
{
	...
	while(LL_I2C_IsActiveFlag_BUSY(OLED_I2C));										// 判断总线是否忙碌
	
	LL_I2C_GenerateStartCondition(OLED_I2C);								
	while(!I2C_CheckEvent(OLED_I2C,I2C_EVENT_MASTER_MODE_SELECT))					// 读SR1->SB位，判断起始位是否发送
		{if (++timeout > TIMEOUT_MAX) return;}									// 超时退出
	
	LL_I2C_TransmitData8(OLED_I2C,0x78);											// 发送地址
	while(!I2C_CheckEvent(OLED_I2C,I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))		// 等待EV6完成；读SR1、SR2
		{if (++timeout > TIMEOUT_MAX) return;}									// 超时退出
	
	LL_I2C_TransmitData8(OLED_I2C,0x00);											// 发送命令
	while(!I2C_CheckEvent(OLED_I2C,I2C_EVENT_MASTER_BYTE_TRANSMITTED))
		{if (++timeout > TIMEOUT_MAX) return;}									// 超时退出
	
	LL_I2C_TransmitData8(OLED_I2C,cmd);												// 发送数据
	while(!I2C_CheckEvent(OLED_I2C,I2C_EVENT_MASTER_BYTE_TRANSMITTED))
		{if (++timeout > TIMEOUT_MAX) return;}									// 超时退出
...
void OLED_WR_Data(uint8_t *dat,uint8_t len)
{
	...
	LL_I2C_TransmitData8(OLED_I2C,0x40);										// 发送数据
	while(!I2C_CheckEvent(OLED_I2C,I2C_EVENT_MASTER_BYTE_TRANSMITTED))
		{if (++timeout > TIMEOUT_MAX) return;}									// 超时退出
	...
	{
		LL_I2C_TransmitData8(OLED_I2C, dat[i]);									// 发送当前字节
		while(!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED))		// 等待字节发送完成
			{if (++timeout > TIMEOUT_MAX) return;}								// 超时退出
	}
...
void OLED_Init(void)
{
	...
	OLED_WR_Cmd(0xAE);	// 设置显示开启/关闭，0xAE关闭，0xAF开启
	
	OLED_WR_Cmd(0xD5);	// 设置显示时钟分频比/振荡器频率
	OLED_WR_Cmd(0x80);	// 0x00~0xFF
	
	OLED_WR_Cmd(0xA8);	// 设置多路复用率
	OLED_WR_Cmd(0x3F);	// 0x0E~0x3F
	
	OLED_WR_Cmd(0xD3);	// 设置显示偏移
	OLED_WR_Cmd(0x00);	// 0x00~0x7F
	
	OLED_WR_Cmd(0x40);	// 设置显示开始行，0x40~0x7F
	...
	OLED_WR_Cmd(0xA6);	// 设置正常/反色显示，0xA6正常，0xA7反色
...
void OLED_SetCursor(uint8_t Page, uint8_t X)
{
	...
	/*通过指令设置页地址和列地址*/
	OLED_WR_Cmd(0xB0 | Page);					//设置页位置
	OLED_WR_Cmd(0x10 | ((X & 0xF0) >> 4));	//设置X位置高4位
	OLED_WR_Cmd(0x00 | (X & 0x0F));			//设置X位置低4位
}
...
void OLED_Update(void)
{
	...
	for (j = 0; j < 8; j ++)
	{
		...
		OLED_WR_Data(OLED_DisplayBuf[j], 128);
	}
}
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			...
			OLED_WR_Data(&OLED_DisplayBuf[j][X], Width);
		}
	}
}
